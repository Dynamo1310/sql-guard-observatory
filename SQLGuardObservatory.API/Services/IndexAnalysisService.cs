...
public class IndexAnalysisService : IIndexAnalysisService
{
    ...
    public async Task<FullIndexAnalysisDto> GetFullAnalysisAsync(IndexAnalysisRequest request)
    {
        ...
        var fragmentedTask = GetFragmentedIndexesAsync(request.InstanceName, request.DatabaseName, request.MinPageCount, request.MinFragmentationPct);
        var unusedTask = GetUnusedIndexesAsync(request.InstanceName, request.DatabaseName, request.MinPageCount);
        var duplicateTask = GetDuplicateIndexesAsync(request.InstanceName, request.DatabaseName);
        var missingTask = GetMissingIndexesAsync(request.InstanceName, request.DatabaseName);
        var disabledTask = GetDisabledIndexesAsync(request.InstanceName, request.DatabaseName);
        var overlappingTask = GetOverlappingIndexesAsync(request.InstanceName, request.DatabaseName);
        var badTask = GetBadIndexesAsync(request.InstanceName, request.DatabaseName);
    ...
    public async Task<bool> TestConnectionAsync(string instanceName)
    {
        try
        {
            var connectionString = BuildConnectionString(instanceName, "master");
    ...
    {
        // Usar Integrated Security para Windows Authentication
        return $"Server={instanceName};Database={databaseName};Integrated Security=True;TrustServerCertificate=True;Connect Timeout=30;Application Name=SQLNovaIndexAnalysis";
    }
    ...
    {
        var connectionString = BuildConnectionString(instanceName, databaseName);
