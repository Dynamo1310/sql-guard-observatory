<#  
    ...
    - Estados: Saludable (>20%), Advertencia (10-20%), Crítico (<10%)
...
BEGIN
    ...
    CREATE INDEX IX_$SqlTable`_Servidor_Drive ON [$SqlSchema].[$SqlTable] ([Servidor], [Drive], [CaptureDate]);
    CREATE INDEX IX_$SqlTable`_Estado ON [$SqlSchema].[$SqlTable] ([Estado], [CaptureDate]);
END
"@
...
if ($TestMode) {
    ...
    Write-Host "      ✓ $($instancesFiltered.Count) instancias ON-PREMISES a procesar (MODO PRUEBA)" -ForegroundColor Yellow
} else {
    Write-Host "      ✓ $($instancesFiltered.Count) instancias ON-PREMISES a procesar" -ForegroundColor Green
}
...
foreach ($inst in $instancesFiltered) {
    ...
    try {
        # Obtener información de discos usando dbatools
        ...
        foreach ($disk in $disks) {
            ...
            $totalGB = [decimal]($disk.Capacity / 1GB)
            $libreGB = [decimal]($disk.Free / 1GB)
        ...
        # Mostrar resumen con colores según estado
        $criticos = ($disks | Where-Object { (($_.Free / $_.Capacity) * 100) -lt 10 }).Count
        $advertencias = ($disks | Where-Object { $pct = (($_.Free / $_.Capacity) * 100); $pct -ge 10 -and $pct -le 20 }).Count
        
        if ($criticos -gt 0) {
            Write-Host " - $diskCount discos ($criticos críticos)" -ForegroundColor Red
        } elseif ($advertencias -gt 0) {
            Write-Host " - $diskCount discos ($advertencias advertencias)" -ForegroundColor Yellow
        } else {
            Write-Host " - $diskCount discos" -ForegroundColor Green
        }
...
# 6. Resumen con estadísticas de estados
$estadoCritico = ($allResults | Where-Object { $_.Estado -eq "Crítico" }).Count
...
Write-Host "  Críticos (<10%):      $estadoCritico" -ForegroundColor $(if ($estadoCritico -gt 0) {"Red"} else {"Green"})
