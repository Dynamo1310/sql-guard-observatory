/**
 * Página de Inventario de Instancias Redis
...
import { redisInventoryApi, CacheMetadata, PaginationInfo } from '@/services/redisInventoryApi';
import { RedisInstance } from '@/types';

// Color por versión de Redis
const getVersionColor = (version: string): string => {
  const majorVersion = parseFloat(version);
  if (majorVersion >= 7) return 'bg-violet-500/10 text-violet-500 border-violet-500/30';
  if (majorVersion >= 6) return 'bg-blue-500/10 text-blue-500 border-blue-500/30';
  if (majorVersion >= 5) return 'bg-cyan-500/10 text-cyan-500 border-cyan-500/30';
...
export default function RedisInstances() {
  ...
  const fetchData = useCallback(async (page: number = 1, pageSize: number = 50) => {
    setLoading(true);
    try {
      const response = await redisInventoryApi.getInstances({
        page,
        pageSize,
      ...
      toast({
        title: 'Error',
        description: 'No se pudieron cargar las instancias de Redis',
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
  ...
  const refreshData = useCallback(async () => {
    ...
    try {
      const response = await redisInventoryApi.refreshInstances();
      setInstances(response.data);
      setCacheInfo(response.cacheInfo);
      setPagination(response.pagination);
      toast({
  ...
  const stats = useMemo(() => {
    ...
    const byVersion = instances.reduce((acc, i) => {
      const v = i.ProductVersion.split('.')[0];
      acc[v] = (acc[v] || 0) + 1;
      return acc;
    }, {} as Record<string, number>);
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight flex items-center gap-2">
            <RedisIcon className="h-8 w-8" />
            Inventario Redis - Instancias
          </h1>
          <p className="text-muted-foreground">
            Listado de todas las instancias Redis registradas
          </p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">AWS ElastiCache</CardTitle>
      ...
      <div className="flex flex-wrap gap-4">
        <div className="relative flex-1 min-w-[200px] max-w-sm">
          ...
          <Input
            placeholder="Buscar cluster o descripción..."
            value={searchQuery}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Información detallada de cada cluster Redis
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className={cn("relative", loading && "opacity-50")}>
            ...
            {instances.length === 0 ? (
              <div className="text-center py-12">
                <RedisIcon className="h-12 w-12 mx-auto mb-4" />
                ...
                <TableBody>
                  {instances.map((instance) => (
                    <TableRow key={instance.id}>
                      ...
                      <TableCell className="text-sm text-muted-foreground max-w-[200px] truncate" title={instance.Description}>
